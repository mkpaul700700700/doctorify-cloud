...
import { Calendar, Clock, Activity, FilePlus, UserCog, Video } from "lucide-react"
...
import { updateSignature } from "@/app/actions/updateProfileImages"
...
export default async function DashboardContent({
  ...
  // Fetch full user (for signature URL etc.)
  ...
  // Synchronous Stripe Verification
  if (paymentParam === "success" && sessionIdParam) {
    try {
      ...
      const checkoutSession = await stripe.checkout.sessions.retrieve(sessionIdParam)
      if (checkoutSession.payment_status === "paid" && checkoutSession.metadata?.appointmentId) {
        ...
        if (existingAppt && existingAppt.status !== "CONFIRMED") {
          await prisma.appointment.update({ where: { id: appointmentId }, data: { paymentStatus: "PAID", status: "CONFIRMED" } })
          await prisma.inAppNotification.create({ data: { userId: existingAppt.doctorId, message: `New paid appointment confirmed from ${existingAppt.patient.name} for ${existingAppt.date.toISOString().split('T')[0]} at ${existingAppt.time}.` } })
          await prisma.inAppNotification.create({ data: { userId: existingAppt.patientId, message: `Your payment was successful! Appointment with Dr. ${existingAppt.doctor.name} is confirmed.` } })
          const dateStr = existingAppt.date.toISOString().split('T')[0]
          await sendNotificationEmail(existingAppt.doctor.name || "Doctor", existingAppt.doctor.email || "", "Payment Confirmed - New Appointment - Doctorify", `<p>You have a new paid appointment from <strong>${existingAppt.patient.name}</strong>.</p><p>Date: ${dateStr} at ${existingAppt.time}</p>`)
          await sendNotificationEmail(existingAppt.patient.name || "Patient", existingAppt.patient.email || "", "Payment Successful - Doctorify", `<p>Your payment was successful!</p><p>Your appointment with Dr. ${existingAppt.doctor.name} is now confirmed for <strong>${dateStr} at ${existingAppt.time}</strong>.</p>`)
        }
      }
    } catch (e) {
  ...
  if (role === "PATIENT") {
    ...
    medicalRecord = await prisma.medicalRecord.findUnique({ where: { patientId: userId } })
    medicalReports = await prisma.medicalReport.findMany({ where: { patientId: userId }, orderBy: { createdAt: "desc" } })
  } else if (role === "DOCTOR") {
    appointments = await prisma.appointment.findMany({
    ...
    const docProfile = await prisma.doctorProfile.findUnique({ where: { userId } })
    isVerifiedDoctor = docProfile?.isVerified || false
    doctorSchedules = await prisma.doctorSchedule.findMany({ where: { doctorId: userId } })
    specialSchedules = await prisma.specialSchedule.findMany({ where: { doctorId: userId }, orderBy: { date: "asc" } })
  } else if (role === "ADMIN") {
    appointments = await prisma.appointment.findMany({ include: { doctor: true, patient: true }, orderBy: { date: "desc" } })
    allUsers = await prisma.user.findMany({ orderBy: { role: "asc" } })
    unverifiedDoctors = await prisma.doctorProfile.findMany({ where: { isVerified: false }, include: { user: true } })
  }
  ...
  if (role === "ADMIN") {
    ...
    const totalRevenue = appointments.filter(a => a.paymentStatus === "PAID" && a.amount).reduce((sum, a) => sum + (a.amount || 0), 0)
      ...
      <div className={styles.container}>
        ...
        <div className={styles.statsGrid}>
          <div className={styles.statCard}>
            <div className={styles.statIcon}><span style={{ fontSize: "24px", fontWeight: "bold" }}>👤</span></div>
            <div><div className={styles.statValue}>{totalPatients}</div><div className={styles.statLabel}>Registered Patients</div></div>
          </div>
          <div className={styles.statCard}>
            <div className={styles.statIcon}><span style={{ fontSize: "24px", fontWeight: "bold" }}>🩺</span></div>
            <div><div className={styles.statValue}>{totalDoctors}</div><div className={styles.statLabel}>Active Doctors</div></div>
          </div>
          <div className={styles.statCard}>
            <div className={styles.statIcon} style={{ background: "rgba(16, 185, 129, 0.1)", color: "var(--primary-color)" }}>
              <span style={{ fontSize: "24px", fontWeight: "bold" }}>৳</span>
            </div>
            <div className={styles.statInfo}><div className={styles.statValue}>{totalRevenue.toLocaleString()}</div><div className={styles.statLabel}>Total Revenue Generated</div></div>
          </div>
          <div className={styles.statCard}>
            <div className={styles.statIcon} style={{ background: "rgba(245, 158, 11, 0.1)", color: "var(--warning)" }}>
              <Activity size={24} />
            </div>
            <div className={styles.statInfo}><div className={styles.statValue}>{appointments.filter(a => a.status === 'PENDING').length}</div><div className={styles.statLabel}>Pending Actions</div></div>
          </div>
        </div>
                ...
                {appointments.map(app => (
                  <tr key={app.id} style={{ borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
                    ...
                    <td style={{ padding: "1rem 1.5rem" }}>
                      <span className={`${styles.badge} ${app.status === 'PENDING' ? styles.badgePending : app.status === 'CONFIRMED' ? styles.badgeConfirmed : app.status === 'COMPLETED' ? styles.badgeCompleted : ''}`}>
                        {app.status}
                      </span>
                    </td>
                  </tr>
              ...
              <tbody>
                {allUsers.map(u => (
                  <tr key={u.id} style={{ borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
                    ...
                    <td style={{ padding: "1rem 1.5rem" }}>
                      <span style={{ fontSize: "0.75rem", fontWeight: 600, padding: "0.25rem 0.5rem", borderRadius: "4px", backgroundColor: "rgba(0,0,0,0.05)" }}>{u.role}</span>
                    </td>
    ...
    <div className={styles.container} style={{ paddingTop: 0 }}>
      ...
      {role === "DOCTOR" && isVerifiedDoctor && (
        <div className={styles.section} style={{ marginTop: "2rem" }}>
          ...
          <div style={{ padding: "1.5rem" }}>
            <ScheduleManager regularSchedules={doctorSchedules} specialSchedules={specialSchedules} />
          </div>
        </div>
      )}
                  ...
                  {role === "DOCTOR" && app.patient.medicalReports && app.patient.medicalReports.length > 0 && (
                    <div style={{ marginTop: "0.5rem", fontSize: "0.8rem" }}>
                      <strong>Files:</strong> {app.patient.medicalReports.map((r: any) => (
                        <a key={r.id} href={r.fileUrl} target="_blank" rel="noopener noreferrer" style={{ color: "var(--primary)", marginRight: "0.5rem", textDecoration: "underline" }}>{r.fileName}</a>
                      ))}
                    </div>
                  )}

                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <span className={`${styles.badge} ${app.status === 'PENDING' ? styles.badgePending : app.status === 'CONFIRMED' ? styles.badgeConfirmed : app.status === 'CANCELLED_EMERGENCY' ? styles.badgeCompleted : ''}`}>
                    ...
                    {app.status === "CONFIRMED" && (
                      <JoinConsultationButton appointmentId={app.id} dateStr={app.date.toISOString()} timeStr={app.time} />
                    )}
          ...
          <ul className={styles.appointmentList}>
            {past.map(app => (
              <li key={app.id} className={styles.appointmentItem}>
                ...
                <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
                  <span className={`${styles.badge} ${styles.badgeCompleted}`}>{app.status}</span>
                  {false && <ChatHistoryModal appointmentId={app.id} currentUserId={userId} role={role} />}
